    ...
    movePiece = (selectedId, finalPosition, currentGame, isMyMove) => {
        ...
        if(isMyMove) {
            socket.emit('new move', {
                nextPlayerColorToMove: !this.state.gameState.thisPlayerIsWhite,
                playerColorThatJustMovedIsWhite: this.state.gameState.thisPlayerIsWhite,
        ...
        for(var i = 0; i < 8; i++) {
            ...
            for(var j = 0; j < 8; j++) {
                if(oldBoard[i][j].getPieceIdOnThisSquare() === selectedId) {
                    tmpBoard[i].push(new Square(j, i, null, oldBoard[i][j].canvasCoords));
                } else {
...
const ChessGameWrapper = (props) => {
    ...
    const [opponentDidJoinTheGame, didJoinTheGame] = React.useState(false);
    ...
    React.useEffect(() => {
        ...
        socket.on('start game', (opponentUserName) => {
            console.log("START! " + props.myUserName + " vs " + opponentUserName);
            if(opponentUserName !== props.myUserName) {
                setUserName(opponentUserName);
                didJoinTheGame(true);
            } else {
                socket.emit('request username', gameid); //p2 asks p1's username
            }
        });

        socket.on('give userName', (socketId) => {
            if (socket.id !== socketId) {
                // p1 gives p2 his username
                socket.emit('received username', {userName: props.myUserName, gameId: gameid});
            }
        });

        socket.on('get Opponent UserName', (data) => {
            // p2 receives p1's username
        ...
        <React.Fragment>
            {opponentDidJoinTheGame ? (
                ...
                <div>
                    ...
                    >Hey <strong>{props.myUserName}</strong>, copy and paste the URL below to send to your friend:</h1>
                    <textarea
